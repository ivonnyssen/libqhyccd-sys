...
type qhyccd_handle = extern "C" fn();
...
extern "C" {
    ...
    */
    fn InitQHYCCDResource() -> u32;
    fn ScanQHYCCD() -> u32;
    fn GetQHYCCDSDKVersion(year: *mut u32, month: *mut u32, day: *mut u32, subday: *mut u32)
        -> u32;
    fn GetQHYCCDChipInfo(
...
mod tests {
    ...
    fn it_works() {
        unsafe {
            ...
            let mut d: u32 = 0;
            let mut s: u32 = 0;
            let result = GetQHYCCDSDKVersion(
                &mut y as *mut u32,
                &mut m as *mut u32,
                &mut d as *mut u32,
                &mut s as *mut u32,
            ...
            assert_eq!(s, 14);
            assert_eq!(result, 0);
            let result = InitQHYCCDResource();
            assert_eq!(result, 0);
            let result = ScanQHYCCD();
            assert_eq!(result, 1);
        }
    }
}
