...
fn main() {
    ...
    let number_of_readout_modes = get_number_of_readout_modes(camera.clone()).unwrap();
    trace!(number_of_readout_modes = ?number_of_readout_modes);
    for i in 0..number_of_readout_modes {
        let name = get_readout_mode_name(camera.clone(), i).unwrap();
        println!("{}: {}", i, name);
        let resolution = get_readout_mode_resolution(camera.clone(), i).unwrap();
        println!("{}: {}, {}", i, resolution.0, resolution.1);
    }

    let read_out_mode = get_readout_mode(camera.clone()).expect("get_readout_mode failed");
